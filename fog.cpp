...
void MyWindow::initMatrices()
{
    ...
    ViewMatrix.lookAt(QVector3D(0.0f, 4.0f, 6.0f), QVector3D(0.0f,2.0f,0.0f), QVector3D(0.0f,1.0f,0.0f));
}
...
void MyWindow::render()
{
    ...
    mProgram->bind();
    {
        ...
        float dist = 0.0f;
        for( int i = 0 ; i < 4; i++ ) {
            ModelMatrixTeapot.setToIdentity();
            ModelMatrixTeapot.translate(dist * 0.6f - 1.0f, 0.0f, -dist);
